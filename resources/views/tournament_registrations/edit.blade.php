    ...
    <main class="container mx-auto my-12 px-4 sm:px-6 lg:px-12">
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg space-y-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                <h2 class="text-3xl font-bold text-gray-800">✏️ Edit Tournament Registration</h2>
            ...
            <!-- Form Section -->
            <form action="{{ route('tournament_registrations.update', $registration->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="bg-indigo-50 p-6 rounded-xl shadow-inner space-y-6">
                    ...
                    <div>
                        ...
                        <select name="user_id" class="w-full p-3 border border-gray-300 rounded-lg text-black focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                    ...
                    <div>
                        ...
                        <select name="tournament_id" class="w-full p-3 border border-gray-300 rounded-lg text-black focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                            ...
                            @foreach ($tournaments as $tournament)
                                <option value="{{ $tournament->id }}" {{ $registration->tournament_id == $tournament->id ? 'selected' : '' }}>
                                    {{ $tournament->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="registration_date" class="w-full p-3 border border-gray-300 rounded-lg text-black focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" value="{{ $registration->registration_date }}" required>
                    </div>
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-calendar-check"></i> Update Registration
                </button>
            </form>
        </div>
    </main>
