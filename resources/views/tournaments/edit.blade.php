            ...
            <form action="{{ route('tournaments.update', $tournament) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" name="entry_fee" class="form-control"
                        value="{{ $tournament->entry_fee }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Status</label>
                    <select name="status" class="form-select" required>
                        <option value="pending" {{ $tournament->status == 'pending' ? 'selected' : '' }}>Pending
                        </option>
                        <option value="upcoming" {{ $tournament->status == 'upcoming' ? 'selected' : '' }}>Upcoming
                        </option>
                        <option value="ongoing" {{ $tournament->status == 'ongoing' ? 'selected' : '' }}>Ongoing
                        </option>
                        <option value="completed" {{ $tournament->status == 'completed' ? 'selected' : '' }}>Completed
                        </option>
                        <option value="canceled" {{ $tournament->status == 'canceled' ? 'selected' : '' }}>Canceled
                        </option>
                    </select>
                </div>
