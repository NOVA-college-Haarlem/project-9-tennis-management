<div class="container">
    ...
    <form action="{{ route('tournament_matches.update', $tournament_match->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" name="scheduled_time" class="form-control" value="{{ old('scheduled_time', \Carbon\Carbon::parse($tournament_match->scheduled_time)->format('Y-m-d\TH:i')) }}" required>
        </div>
...
<div class="form-group" id="score-field">
    ...
    <input type="number" name="player1_score" class="form-control" value="{{ old('player1_score', $tournament_match->player1_score) }}" {{ $tournament_match->status == 'completed' ? '' : 'disabled' }}>

    <label for="player2_score">Player 2 Score</label>
    <input type="number" name="player2_score" class="form-control" value="{{ old('player2_score', $tournament_match->player2_score) }}" {{ $tournament_match->status == 'completed' ? '' : 'disabled' }}>
</div>
