                ...
                <tr>
                    ...
                    <td>
                        @if ($tournament_match->status == 'completed')
                            {{ $tournament_match->player1->firstname }}: {{ $tournament_match->player1_score }} - {{ $tournament_match->player2->firstname }}: {{ $tournament_match->player2_score }}
                        @else
                    ...
                    <td>
                        <a href="{{ route('tournament_matches.show', $tournament_match->id) }}" class="btn btn-info">View</a> 
                        <a href="{{ route('tournament_matches.edit', $tournament_match->id) }}" class="btn btn-warning">Edit</a> 
                        <form action="{{ route('tournament_matches.destroy', $tournament_match->id) }}" method="POST" style="display:inline;"> 
                            @csrf @method('DELETE')
