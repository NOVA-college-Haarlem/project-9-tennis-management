        ...
        <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
            <div class="max-w-6xl mx-auto space-y-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">🎾 Youth Programs</h2>
                    <a href="{{ route('youth_programs.create') }}"
                    ...
                    <div class="bg-indigo-50 shadow-lg rounded-2xl p-6 hover:shadow-2xl transition duration-300">
                        <div class="flex flex-col h-full">
                            <div class="space-y-2 mb-4">
                                <p class="text-lg font-semibold text-gray-800">#{{ $program->id }} – <span
                                        class="text-indigo-600">{{ $program->name }}</span></p>
                                <p class="text-sm text-gray-600"><strong>Age Range:</strong> <span
                                        class="font-medium text-gray-800">{{ $program->min_age }} - {{ $program->max_age }}</span></p>
                                <p class="text-sm text-gray-600"><strong>Schedule:</strong> <span
                                        class="font-medium text-gray-800">{{ $program->schedule }}</span></p>
                                <p class="text-sm text-gray-600"><strong>Fee:</strong> <span
                                        class="font-medium text-gray-800">${{ $program->fee }}</span></p>
                            </div>

                            <div class="mt-auto pt-4 flex flex-wrap gap-2 border-t border-indigo-100">
                                <a href="{{ route('youth_programs.show', $program->id) }}"
                                ...
                                <a href="{{ route('youth_programs.edit', $program->id) }}"
                                ...
                                <form action="{{ route('youth_programs.destroy', $program->id) }}" method="POST"
