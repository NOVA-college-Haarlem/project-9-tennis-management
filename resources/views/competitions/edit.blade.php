<div class="container">
    ...
    <form action="{{ route('competitions.update', $competition->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="start_date" class="form-control" value="{{ old('start_date', $competition->start_date) }}" required>
            @error('start_date') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label for="end_date">End Date</label>
            <input type="date" name="end_date" class="form-control" value="{{ old('end_date', $competition->end_date) }}" required>
            @error('end_date') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label for="min_skill_level">Min Skill Level</label>
            <input type="number" name="min_skill_level" class="form-control" value="{{ old('min_skill_level', $competition->min_skill_level) }}" min="1" required>
            @error('min_skill_level') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
            <label for="max_skill_level">Max Skill Level</label>
            <input type="number" name="max_skill_level" class="form-control" value="{{ old('max_skill_level', $competition->max_skill_level) }}" min="1" required>
            @error('max_skill_level') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="mb-3">
